...
function generateOctaves(
    ...
    return generateOctaves(x, y, total, getNoise2D, currentOctave + 1, value + lastValue)
}
...
export default function getNoiseValueAtCoord(
    ...
    let noiseVal = generateOctaves(nx, ny, options.octaves, getNoise2D) * options.maxHeight
    if (options.exponent) {
